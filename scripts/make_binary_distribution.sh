...
mkdir $BASE $BASE/bin $BASE/data $BASE/data/mysql $BASE/data/test  $BASE/man $BASE/man/man1 \
...
for i in sql/ChangeLog COPYING COPYING.LIB README Docs/INSTALL-BINARY \
         ...
         gemini/GPL_LICENSE.txt gemini/NuSphere_Public_License.txt
do
  $CP $i $BASE
done
...
case $PLATFORM in
Linux) nm --numeric-sort --demangle $BASE/bin/mysqld | fgrep -i " T " > $BASE/bin/mysqld.sym
       ;;
esac
...
$CP -r sql/share/* $BASE/share/mysql;$CP gemini/msgs/gemmsgs.db $BASE/share/mysql/english;(cd gemini;make_so;$CP gemini.so $BASE/bin)
rm -f $BASE/share/mysql/Makefile* $BASE/share/mysql/*/*.OLD

$CP mysql-test/*-test-run mysql-test/install_test_db $BASE/mysql-test/
...
$CP scripts/* $BASE/bin;$CP man/*.1 $BASE/man/man1;$CP -r Docs/bychapter $BASE
...
case $PLATFORM in
Linux) NEW_NAME=mysql@MYSQL_SERVER_SUFFIX@-$version-i386-pc-linux-gnu
       ;;
SunOS) NEW_NAME=mysql@MYSQL_SERVER_SUFFIX@-$version-sparc-sun-solaris2.6
       ;;
